...
public class PrintPic {
    ...
    /**
     * Imprimir usando mapas de bits ráster
     *
     * @return Byte
     */
    public byte[] printDraw() {
        ...
        // Instrucciones para imprimir mapas de bits ráster
        imgbuf[0] = 29;// Hexágono 0x1D
        imgbuf[1] = 118;// Hex 0x76
        imgbuf[2] = 48;// 30
        imgbuf[3] = 0;// Modo de mapa de bits 0,1,2,3
        // Indica el número de bits de mapa de bits en la dirección horizontal（xL+xH × 256）
        imgbuf[4] = (byte) (this.width / 8);
        imgbuf[5] = 0;
        // Indica el número de puntos de mapa de bits en la dirección vertical（ yL+ yH × 256）
        imgbuf[6] = (byte) (getLength() % 256);//
        imgbuf[7] = (byte) (getLength() / 256);

        s = 7;
        for (int i = 0; i < getLength(); i++) {// La altura del mapa de bits de bucle
            for (int k = 0; k < this.width / 8; k++) {// El ancho del mapa de bits de bucle
                int c0 = nbm.getPixel(k * 8, i);// Devuelve el color de las coordenadas especificadas
                int p0;
                if (c0 == -1)// Determine si el color es blanco
                    p0 = 0;// 0,No imprima este punto
                else {
                    p0 = 1;// 1,Imprime este punto
                }
